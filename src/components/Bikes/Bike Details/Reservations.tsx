...
const Reservation = ({ data }) => {
  ...
  const filteredUser = state.allUsers.filter(
    (u) => u.email === data.reservedBy
  );
  const user = filteredUser.length ? filteredUser[0] : null;
  return (
    <Paper style={{ marginTop: "10px" }}>
      ...
      <Box style={{ display: "flex" }}>
        <Box style={{ width: "50%", borderRight: "1px solid #efefef" }}>
          {userAttributes.map((u, i) => {
            return (
              <Attribute key={i}>
                <AttributeLabel>{u.label}</AttributeLabel>
                <AttributeValue>{user[u.field]}</AttributeValue>
              </Attribute>
            );
          })}
        </Box>
